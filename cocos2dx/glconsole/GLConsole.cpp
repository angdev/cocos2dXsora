...
inline void GLConsole::Init()
{
	...
	m_nTextHeight = GLFont::Last()->CharHeight();
	m_nTextWidth = GLFont::Last()->CharWidth();
	m_nScrollPixels = 0;
	m_nCommandNum = 0;
	...
	//m_Viewport.x = CCDirector::sharedDirector()->getWinSizeInPixels().width;
	m_Viewport.x = 0;
	m_Viewport.y = 0;
	m_Viewport.width = (int)CCDirector::sharedDirector()->getWinSizeInPixels().width;
	m_Viewport.height = (int)CCDirector::sharedDirector()->getWinSizeInPixels().height;
...
inline int GLConsole::_GetConsoleHeight()
{
	...
	// animating
	if( m_bIsChanging ) {
		float elapsed = (float)m_Timer.Elapsed();
		if( m_bConsoleOpen ) { // closing motion
...
inline void GLConsole::RenderConsole()
{
	...
	if( m_bConsoleOpen || m_bIsChanging ) {
        ...
        kmMat4 orthoMatrix;
        kmMat4OrthographicProjection(&orthoMatrix, 0,(float)m_Viewport.width,0,(float)m_Viewport.height,-1,1);
        ...
        CCDirector::sharedDirector()->setScissorTest(true);

		//rend67er transparent background
        //어차피 위에서 이미 깊이가 꺼져잇다
...

		{
            ...
            //quad에 걸맞는 사각형 만들기
            float color_data[] = {
                m_consoleColor.r, m_consoleColor.g, m_consoleColor.b, m_consoleColor.a,
                m_consoleColor.r, m_consoleColor.g, m_consoleColor.b, m_consoleColor.a,
                m_consoleColor.r, m_consoleColor.g, m_consoleColor.b, m_consoleColor.a,
                m_consoleColor.r, m_consoleColor.g, m_consoleColor.b, m_consoleColor.a,
            };
			...
			float vertex_data[] = {
				0, 0, 
				(float)m_Viewport.width-1, 0,
				(float)m_Viewport.width-1, (float)m_Viewport.height-1,
				0, (float)m_Viewport.height-1,
			};

            glVertexAttribPointer(kCCVertexAttrib_Position, 2, GL_FLOAT, GL_FALSE, 0, vertex_data);
            glVertexAttribPointer(kCCVertexAttrib_Color, 4, GL_FLOAT, GL_FALSE, 0, color_data);
		...
		//가위테스트 올린거 내리기
        CCDirector::sharedDirector()->setScissorTest(false);
	}
}
...
{
	float elapsed = (float)m_BlinkTimer.Elapsed();
...
inline void GLConsole::_RenderText()
{
	...
	CCDirector::sharedDirector()->setScissorTest(true);
    {
		...
		glColor4f(m_commandColor.r, m_commandColor.g, m_commandColor.b, 1.0f);
		int size = m_sCurrentCommandBeg.length();
		std::string em = "";
		for(int i=0;i<size;i++) {
			em = em+" ";
		}
		m_pGLFont->glPrintf( m_nConsoleLeftMargin, lineLoc - m_nScrollPixels, "> " + em + blink );
		m_pGLFont->glPrintf( m_nConsoleLeftMargin, lineLoc - m_nScrollPixels, "> " + em + m_sCurrentCommandEnd );
		//텍스쳐에 알파값이 없을때는 순수 문자열을 가장 나중에 찍어야한다
		m_pGLFont->glPrintf( m_nConsoleLeftMargin, lineLoc - m_nScrollPixels, "> " + m_sCurrentCommandBeg );
		...
		for(  int i = 1 ; i < lines; i++ ) {	
			...
			if( (int)m_consoleText.size() > i - 1 ) {
                ...
                getShaderProgram()->setUniformLocationWith4fv(m_nUniformColor, (float*)&color.fColor, 1);

				//wrap text to multiple lines if necessary
				//int chars_per_line = (int)(1.65*m_Viewport.width / m_nTextHeight);
				int chars_per_line = (int)(1.65*m_Viewport.width / m_nTextWidth);
				if( chars_per_line == 0 ) {
...
* Process key presses
*/
inline void GLConsole::KeyboardFunc( unsigned char key, int state)
{
	if(!IsOpen())
		return;
	...
	bool shift_down = keyboard.IsKeyDown(CCConsoleKeyboard::kKeyShift) == CCConsoleKeyboard::kPress;
	bool ctrl_down = keyboard.IsKeyDown(CCConsoleKeyboard::kKeyCtrl) == CCConsoleKeyboard::kPress;
	...
	if(true) {
		...
		case CCConsoleKeyboard::kKeyLeft:
			if(state == 1) { CursorLeft(); }
			break;
		case CCConsoleKeyboard::kKeyRight:
			if(state == 1) { CursorRight(); }
			break;
		case CCConsoleKeyboard::kKeyPageUp:
			if(state == 1) { ScrollDownPage(); }
			break;
		case CCConsoleKeyboard::kKeyPageDown:
			if(state == 1) { ScrollUpPage(); }
			break;
		case CCConsoleKeyboard::kKeyUp:
			if(state == 1) { HistoryBack(); }
			break;
		case CCConsoleKeyboard::kKeyDown: 
			if(state == 1) { HistoryForward(); }
			break;
		case CCConsoleKeyboard::kKeyHome:
			if(state == 1) { CursorToBeginningOfLine(); }
			break;
		case CCConsoleKeyboard::kKeyEnd:
			if(state == 1) { CursorToEndOfLine(); }
			break;
		default:
...
GLConsole* GetConsole( GLConsole* pFirstConsole)
{
